    ...
    <t-form ref="form" :data="formData" :label-width="80" colon @reset="onReset" @submit="onSubmit">
      <t-row>
        <t-col :span="10">
          <t-row :gutter="[24, 24]">
            <t-col :span="4">
              <t-form-item label="客户编号" name="id">
                <t-input
                  v-model="formData.id"
            ...
            <t-col :span="4">
              <t-form-item label="客户姓名" name="pid">
                <t-input
...
import { DeleteRecord, getClientList, getList, getOrderDetail, getOrderList, Search } from '@/api/list';
import Trend from '@/components/trend/index.vue';
import { prefix } from '@/config/global';
import { CONTRACT_PAYMENT_TYPES, CONTRACT_STATUS, CONTRACT_TYPE_OPTIONS, CONTRACT_TYPES } from '@/constants';
import { PARTY_B_OPTIONS } from '@/pages/form/base/constants';
...
const COLUMNS: PrimaryTableCol[] = [
  ...
  { title: '客户姓名', colKey: 'name', width: 160 },
  {
    title: '客户地址',
    width: 160,
    ellipsis: true,
...
const OnSearch = async () => {
  ...
  try {
    ...
    const list = await Search(result.id, result.pid, result.cid, result.sid);
    // const { list } = await getList();
    console.log(list);
    data.value = list;
    console.log(data);
...
const rehandleClickOp = async (ctx: unknown) => {
  ...
  const list = await getOrderDetail(row_id);
  // const { list } = await getList();
  console.log(list);
  console.log(list.id);
  pagination.value = {
  ...
  BASE_INFO_DATA.value = [
    ...
    {
      name: '预定时间',
      value: list.book_time,
      type: null,
    },
    {
      name: '发货时间',
      value: list.order_time,
      type: null,
    },
    {
      name: '付款方式',
