...
import { getInDetail, getInList, getList, InDelete, InSearch } from '@/api/list';
import Trend from '@/components/trend/index.vue';
import { prefix } from '@/config/global';
import { CONTRACT_PAYMENT_TYPES, CONTRACT_STATUS, CONTRACT_TYPE_OPTIONS, CONTRACT_TYPES } from '@/constants';
import { CATEGORY_OPTIONS, PARTY_B_OPTIONS } from '@/pages/in_form/base/constants';
...
const OnSearch = async () => {
  ...
  try {
    ...
    const list = await InSearch(result.id, result.pid, result.sid, result.category);
    // const { list } = await getList();
    console.log(list);
    data.value = list;
    console.log(data);
...
const rehandleClickOp = async (ctx: unknown) => {
  ...
  const list = await getInDetail(row_id);
  // const { list } = await getList();
  console.log(list);
  console.log(list.id);
  pagination.value = {
